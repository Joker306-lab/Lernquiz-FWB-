...
function updateModeUI(mode){
  ...
  document.getElementById("activeModeBadge").textContent=mode==="learn"?"Lernmodus aktiv":"Prüfungsmodus aktiv";
}
document.getElementById("categoryMenu").innerHTML=categoryCards.map(createCard).join("");
document.getElementById("explainMenu").innerHTML=explainCards.map(item=>`<div class="menu-card ${item.accent}"><span class="number">${item.number}</span><div><h2>${item.title}</h2><p>${item.description}</p></div></div>`).join("");
